import { CanvasRef, NativeCanvas } from "react-native-wgpu";
import { config } from "./utils/tools";
import { runOnUI } from "react-native-reanimated";

export type SharedContext = {
  lastPosition: { x: number; y: number } | null;
  gestureCallback: (x: number, y: number) => void;
};

export async function init(ref: React.RefObject<CanvasRef>, sharedContext: { value: SharedContext }) {
  if (!navigator.gpu) {
